...
Phoenix 1.0.0 是基于小米 17 Ultra OS4 官方相机的移植项目，面向搭载澎湃 OS3 的 LEICA 联名直板机型。工程包含相机补丁、LSPosed 扩展、根模块资产和统一构建工具，可生成 Camera APK、LSP APK 与 All-in-One 安装包。
...
| 根管理器 | 理论上支持 KernelSU（系）、Magisk、APatch |
...
产物位于 `dist/Phoenix-1.0.0/`：

| 文件 | 内容 |
| --- | --- |
| `Phoenix_Camera_Phoenix-1.0.0.apk` | 移植相机 |
| `Phoenix_LSP_Phoenix-1.0.0.apk` | LSPosed 扩展 |
| `Phoenix_Phoenix-1.0.0_AllInOne.zip` | 两个 APK、模块脚本与资产 |
...
**安装前务必清除“相机”和“小米相册-编辑”的应用数据；安装后严禁清除这两个应用的数据。**
...
```powershell
adb push "dist/Phoenix-1.0.0/Phoenix_Phoenix-1.0.0_AllInOne.zip" /data/local/tmp/Phoenix-1.0.0.zip
adb shell su -c "ksud module install /data/local/tmp/Phoenix-1.0.0.zip"
```
...
LSP 的可编译输入是 smali 与资源，工程不提供等价的 Java/Kotlin 源码项目。
...
输出为 `dist/Phoenix-1.0.0-source.zip`，包含构建输入和文档，不包含 APK、下载工具、编译缓存或签名密钥。
